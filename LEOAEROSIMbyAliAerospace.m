classdef LEOAEROSIMbyAliAerospace < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function plotPressure(app)
            ...
            shadowMask    = app.BlockedPanels | app.BackwardPanels;
            unshadowMask  = ~shadowMask;

            %pressure colour path
            patch(app.PressureAxes, ...
                'Faces', faces(unshadowMask,:), ...
                'Vertices', verts, ...
                'FaceVertexCData', cdata(unshadowMask), ...
                'FaceColor', 'flat', ...
                'EdgeColor', 'none');

            hold(app.PressureAxes, 'on');

            %black for shadowed panels
            patch(app.PressureAxes, ...
                'Faces', faces(shadowMask,:), ...
                'Vertices', verts, ...
                'FaceColor', [0 0 0], ...
                'EdgeColor', 'none');
